...
const csvData = `SamplingTarget,Average Aerror
Height,0.11461983085749998
Ratio,0.12075144608562498
`;
...
const Graph5SamplingTarget = () => {
    ...
    useEffect(() => {
        ...
        const categories = parsedData.map(row => row.SamplingTarget);
        const data = parsedData.map(row => parseFloat(row['Average Aerror']));

        const option = {
            title: {
                text: 'Bar Chart of Average Aerror by SamplingTarget'
            },
            tooltip: {
                trigger: 'axis',
                axisPointer: {
                ...
                formatter: function (params) {
                    var point = params[0];
                    var info = `SamplingTarget: ${categories[point.dataIndex]}<br>Average Aerror: ${point.value.toFixed(4)}`;
                    return info;
                }
            },
            xAxis: {
        ...
        chartInstance.on('click', function (params) {
            ...
            var info = `SamplingTarget: ${categories[point]}<br>Average Aerror: ${data[point].toFixed(4)}`;
