...
const csvData = `ModelName,Average Aerror
ResNet50,0.12658847899749998
VGG19,0.10878279794562497
`;
...
const Graph4ModelName = () => {
    ...
    useEffect(() => {
        ...
        const categories = parsedData.map(row => row.ModelName);
        const data = parsedData.map(row => parseFloat(row['Average Aerror']));

        const option = {
            title: {
                text: 'Bar Chart of Average Aerror by ModelName'
            },
            tooltip: {
                trigger: 'axis',
                axisPointer: {
                ...
                formatter: function (params) {
                    var point = params[0];
                    var info = `ModelName: ${categories[point.dataIndex]}<br>Average Aerror: ${point.value.toFixed(4)}`;
                    return info;
                }
            },
            xAxis: {
        ...
        chartInstance.on('click', function (params) {
            ...
            var info = `ModelName: ${categories[point]}<br>Average Aerror: ${data[point].toFixed(4)}`;
